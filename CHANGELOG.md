...
- A polished, terminal-width-aware ASCII welcome and goodbye screen (`display.startup_banner`/`goodbye_banner`): a hand-drawn SYSAI wordmark that adapts to wide/medium/narrow terminals (falling back to plain text rather than ever overflowing), stays legible under `NO_COLOR`, and shows only status lines that are actually true. The goodbye banner is printed exactly once, from the same place for `sysai stop`, Ctrl+D/EOF, and `exit`; a crash or unexpected shutdown never gets the celebratory form.
...
- README rewritten and substantially shortened around SysAI's current identity as a local Linux intelligence and diagnostic companion (Bash session + diagnostics + history intelligence + local experience memory), rather than describing it primarily as an "AI-aware Bash session" or documenting every implementation detail. Deep technical detail moved to `SECURITY.md` and `CONTRIBUTING.md`, which the README now links to. No behavior changed; this is a documentation/positioning update.
