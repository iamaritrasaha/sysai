...
`memory.py` is a local, structured Experience Engine at `$XDG_STATE_HOME/sysai/memory.db` (mode `0600`, directory mode `0700`). Schema v2 preserves v1 rows through a transactional migration and adds stable fingerprints, independent-session occurrences, lightweight assessment metadata, outcomes, corrections, and fixed relationships. It never stores raw logs, terminal transcripts, full model answers, or model reasoning; every string is redacted and sanitized before being written. Automatic writes come only from Python-controlled deterministic facts/findings and recurrence — never free model text. User feedback and outcomes are user reports, not automatic machine truth. Memory is inert data: it cannot execute a string, supply an action ID or argv, select privilege, or become shell input. Bounded sanitized experience may inform reasoning, but never web search; raw history and the database are never sent remotely. There is no network memory backend or automatic provider fallback.
